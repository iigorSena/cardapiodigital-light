  ...
  if (item.massa) {
      ...
      if (item.massa) {
          ...
          divRecheios.innerHTML = `
            <p><strong>Escolha o seu recheio</strong></p>
                ...
                ${[
                  ...
                  ...RECHEIOS_GRATIS.map(recheio => `
                    <tr>
                      <td>
                        <label>
                          <input type="checkbox" class="check-recheio"
                            data-index="${index}" data-nome="${recheio}" data-valor="0"
                            ${item.recheiosExtras?.some(r => r.nome === recheio) ? 'checked' : ''}>
                          ${recheio}
                        </label>
                      </td>
                      <td class="cln-preco">Grátis</td>
                    </tr>
                  `),

                  // Só exibe recheios pagos se o array não for vazio/vazio
                  ...(Array.isArray(RECHEIOS_PAGOS) 
                    ? RECHEIOS_PAGOS.filter(r => r.nome && r.valor > 0).map(recheio => `
                        <tr>
                          <td>
                            <label>
                              <input type="checkbox" class="check-recheio"
                                data-index="${index}" data-nome="${recheio.nome}" data-valor="${recheio.valor}"
                                ${item.recheiosExtras?.some(r => r.nome === recheio.nome) ? 'checked' : ''}>
                              ${recheio.nome}
                            </label>
                          </td>
                          <td class="cln-preco">R$ ${recheio.valor.toFixed(2).replace('.', ',')}</td>
                        </tr>
                    `)
                    : []
                  )
...
// Checkbox de recheios
  document.querySelectorAll('.check-recheio').forEach(checkbox => {
    checkbox.addEventListener('change', () => {
      ...
      if (checkbox.checked && recheiosSelecionados.length >= 1) {
        checkbox.checked = false;
        alert('Você só pode selecionar até 1 recheio.');
        return;
      }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let mensagem = '*Pedido Doce Pecado*%0A%0A';
  let totalCentavos = 0;
